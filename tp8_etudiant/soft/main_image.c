...
__attribute__ ((constructor)) void main()
{
    ...
    int		npixels;		// number of pixels per image		
    int		nblocks; 		// number of blocks per image
    ...
    while(image < 20)
    {
        ...
        if ( ioc_read( base + pid * nblocks, buf_in + pid * npixels, nblocks) )                                  // lba: index de premier block, premier: 0, deuxième: 0+32 ...
        {
        ...
        if ( fb_sync_write(pid * npixels, buf_out + pid * npixels, npixels) )                              // offset pour framebuffer est toujours 0, longeur de block c'est npixels
        { 
